...
// // void buildRPC(stringstream& ss) {
// //     ss << endl;
// // }

// template <typename T> void buildRPC(stringstream& rpc, string method, int id, const First& first, const Rest&... rest) 
// {
...
// template <typename T> void addParam(stringstream& rpc, const T& t) 
// {
...
// template <typename First, typename... Rest> void addParam(stringstream& rpc, const First& first, const Rest&... rest) 
// {
//     rpc << first << ", ";
//     addParam(rpc, rest...); // recursive call using pack expansion syntax
// }

void delete_cJSON(cJSON* obj)
{
    //std::cout << "Deleting cJSON\n";
...
RPCObj createRPCObj(string method, int id)
{
    ...
    cJSON_AddItemToObject(RPC_Obj, "params", params = cJSON_CreateObject());
...
void addParam(RPCObj RPC_Obj, string key, int param)
{
    ...
    cJSON_AddItemToObject(params, key.c_str(), cJSON_CreateNumber(param));
}
...
void addParam(RPCObj RPC_Obj, string key, string param)
{
    ...
    cJSON_AddItemToObject(params, key.c_str(), cJSON_CreateString(param.c_str()));
}
...
void addParam(RPCObj RPC_Obj, string key, bool param)
{
    ...
    if (param)
        cJSON_AddItemToObject(params, key.c_str(), cJSON_CreateTrue());
    else    
        cJSON_AddItemToObject(params, key.c_str(), cJSON_CreateFalse());
}
...
/*
Get an int from a json object, returning a default value
if the key is not found.
*/
bool getAttributeDefault_Bool(cJSON* JSONObj, string AttributeKey, bool _default)
{
    bool temp;
    if (!getAttributeValue_Bool(JSONObj, AttributeKey, temp))
    {
        return _default;
    }
    return temp;
...
bool JSONToMap_Str_Str(cJSON *JSONObj, map<string, string> *map_str_str)
{
    cJSON *item;
    std::stringstream ss;
    map<string, string>::iterator findIter;

    if ((!JSONObj) || (!map_str_str))
        return false;
    ...
    while (item)
    {
        ...
        findIter = map_str_str->find(ss.str());

        if (findIter == map_str_str->end())
        {
            // if the key does not exist, insert as new key/value
            map_str_str->insert(make_pair(ss.str(), item->valuestring));
        }
        else
        {
            // if the key does exist, replace the value
            findIter->second = item->valuestring;
        }
        item = item->next;
    }
    return true;
...
bool mergeJSONObjects(cJSON *targetJSONObj, cJSON *sourceJSONObj, bool overwrite)
{
    bool ret(false);
    cJSON* item;
    cJSON* tmpJSON;
    ...
    while (item)
    {
        ...
        {
        case cJSON_False:
            if ((doesAttributeExist(targetJSONObj, item->string)) && overwrite)
